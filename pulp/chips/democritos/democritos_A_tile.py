...
class Democritos_A_TileTcdm(gvsoc.systree.Component):
    def __init__(self, parent, name, parser):
        ...
        # 1 master: PCM HWPE
        pcm_hwpe_masters = 1
        pcm_interleaver = HWPEInterleaver(self, "pcm_interleaver", nb_master_ports=pcm_hwpe_masters, nb_banks=nb_banks, bank_width=4)
        ...
        for i in range(nb_banks):
            ...
            self.bind(pcm_interleaver, f'out_{i}', bank, 'input')
        ...
        for i in range(pcm_hwpe_masters):
            self.bind(self, f'PCM_HWPE_input', pcm_interleaver, f'input')
    ...
    def i_PCM_HWPE_INPUT(self) -> gvsoc.systree.SlaveItf:
        return gvsoc.systree.SlaveItf(self, f'PCM_HWPE_input', signature='io')

class Democritos_A_Tile(gvsoc.systree.Component):
    def __init__(self, parent, name, parser, tid: int=0, weights_path: str=None):
        super().__init__(parent, name)
