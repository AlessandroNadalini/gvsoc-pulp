...
from pulp.chips.megatron.l1_subsystem import L1_subsystem
...
class Cluster(st.Component):
    ...
    def __init__(self, parent, name, config_file, cid: int=0, pulpnn=False):
        ...
        pcm_irq             = cluster_conf.get_property('pe/irq').index('acc_0')
        softex_irq          = cluster_conf.get_property('pe/irq').index('acc_1')
        ...
        periph_ico.add_mapping('pcm', **self._reloc_mapping(cluster_conf.get_property('peripherals/pcm/mapping')))
        self.bind(periph_ico, 'pcm', pcm, 'hwpe_slv')

        periph_ico.add_mapping('softex', **self._reloc_mapping(cluster_conf.get_property('peripherals/softex/mapping')))
        self.bind(periph_ico, 'softex', softex, 'input')
        ...
        # SoftEx HWPE
        for i in range(0, nb_pe):
            self.bind(softex, 'irq', event_unit, 'in_event_%d_pe_%d' % (softex_irq, i))

        self.bind(softex, 'out', l1, 'softex_in')
