...
class L1_subsystem(st.Component):
    ...
    def __init__(self, parent, name, cluster, cluster_conf):
        ...
        l1_interleaver_nb_masters = nb_pe + 4 + 2 # 1 port per PE + 4 for DMA + 1 for PCM HWPE + 1 for SoftEx
        ...
        self.bind(self, 'pcm_in', interleaver, 'in_%d' % (nb_pe + 4))
        self.bind(self, 'softex_in', interleaver, 'in_%d' % (nb_pe + 5))
